...
// With modifications by Carlo Teubner
// Licence: GPLv2+
...
function init(extensionMeta) {
    ...
    EndSessionDialog.EndSessionDialog.prototype._onRebootLinux = function() {
        Gio.file_new_for_path("/boot/control/linux.cfg").copy(
            Gio.file_new_for_path("/boot/control/grub.cfg"),
            Gio.FileCopyFlags.OVERWRITE, null, function () {}, null);
        this._confirm('ConfirmedReboot');
    };

    EndSessionDialog.EndSessionDialog.prototype._onRebootToWindows = function() {
        Gio.file_new_for_path("/boot/control/windows.cfg").copy(
            Gio.file_new_for_path("/boot/control/grub.cfg"),
            Gio.FileCopyFlags.OVERWRITE, null, function () {}, null);
        this._confirm('ConfirmedReboot');
    };
    ...
    EndSessionDialog.EndSessionDialog.prototype._updateButtons = function() {
        ...
        if ( dialogContent.secondaryButtons ) {
            for (let i = 0; i < dialogContent.secondaryButtons.length; i++) {
                ...
                else if ( signal == 'ConfirmedToLinux' ) {
                    buttons.push({ action: Lang.bind(this, this._onRebootLinux),
                                   label: label });
                }
                else if ( signal == 'ConfirmedToWindows' ) {
                    buttons.push({ action: Lang.bind(this, this._onRebootToWindows),
                                   label: label });
                }
                else {
    ...
    const shutdownDialogContent = {
        ...
        secondaryButtons: [{ signal: 'ConfirmedSuspend',
                         ...
                         { signal: 'ConfirmedHibernate',
                           label:  _f("Hibernate") },
                         { signal: 'ConfirmedToLinux',
                           label: "Reboot Linux" },
                         { signal: 'ConfirmedToWindows',
